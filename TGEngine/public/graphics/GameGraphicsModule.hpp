...
namespace tge::graphics {
	...
	public:
		...
		void addTranslationToNodes(std::span<const TNodeHolder> nodeIDs,
			const glm::vec3& transform) {
			nodeHolder.changeAll<1>(nodeIDs, [&](auto old) { old.translation += transform; return old; });
			nodeHolder.changeAll<4>(nodeIDs, [](auto old) { return 1; });
		}

		std::vector<NodeTransform> getTransforms(const std::span<const TNodeHolder> nodeIDs) {
			return nodeHolder.get<1>(nodeIDs);
		}

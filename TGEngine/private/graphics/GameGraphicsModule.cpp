...
namespace tge::graphics {
	...
	inline glm::mat4 calculateMatrixSingle(const NodeTransform& transform) {
		...
		return glm::translate(glm::mat4(1.0f), transform.translation) * glm::scale(glm::mat4(1.0f), transform.scale) *
			rotationMatrix;
	}
		...
		for (size_t i = 0; i < size; i++) {
			if (statuses[i] == 1) {
				...
				if (parentID < i) {
					...
					system.normalModel = parentModel.normalModel * glm::inverseTranspose(currentTransforms);
				}
				else {
	...
	std::vector<TNodeHolder> GameGraphicsModule::addNode(const NodeInfo* nodeInfos,
		...
		BufferInfo bufferInfo{ &*cache, count * sizeof(ValueSystem), DataType::Uniform };
		const auto allData = apiLayer->pushData(1, &bufferInfo).back();

...
#define NO_LAYOUT_GIVEN 4095
	...
	ShaderPipe VulkanShaderModule::compile(const std::vector<ShaderInfo>& vector,
		...
		{
			if (info.name.empty()) {
				PLOG_ERROR << "Shader name is empty! Debug: " << info.debug;
				return {};
			}
			std::lock_guard guard(this->mutex);
			auto permute = this->permuteCache.getGLSLPermute(info.name);
			if (permute) continue;
			if (info.code.empty()) {
				PLOG_ERROR << "Shader code is empty! Name: " << info.name;
				return {};
			}
			std::vector<std::string> codeComplete = { std::string(info.code.begin(),info.code.end()) };
			permute = this->permuteCache.fromStrings(info.name, std::move(codeComplete));			
		}
		...
		for (const auto& info : vector)
		{
			auto shaderLang = getLangFromShaderLang(info.language);
			auto stage = getStageFromLang(shaderLang);
			auto value = this->permuteCache.getGLSLPermute(info.name);
			std::vector<permute::Dependency> dependencies(info.additionalCode.size());
			std::transform(info.additionalCode.begin(), info.additionalCode.end(),
				dependencies.begin(), [](const std::string& str) {
					return permute::Dependency(str, "1");
				});
			std::lock_guard guard(this->mutex);
			GeneralShaderAnalizer analizer(shaderPipe, stage);
			VertexShaderAnalizer vertexAnalizer(shaderPipe, createInfo);
			value->traverser.clear();
			value->traverser.push_back(&analizer);
			if(shaderLang == EShLangVertex) {
				value->traverser.push_back(&vertexAnalizer);
			}
			auto result = value->generate(dependencies);
			value->traverser.clear();
			if (result) {
				shaderPipe->shader.push_back(
					std::pair(result.output, stage));
			}
			else {
				PLOG_ERROR << "Shader compilation failed for " << info.name
					<< ": " << result.error;
				delete shaderPipe;
				return {};
			}
		...
		this->shaderPipes.push_back(shaderPipe);
		return { this->shaderPipes.size() - 1 };
	}
	...
	ShaderPipe VulkanShaderModule::loadShaderPipeAndCompile(
		...
		for (const auto& name : shadernames) {
			const std::string abrivation = name.substr(name.find_last_of('.'));
			const auto& content = util::wholeFile(name);
			if (content.empty()) {
				PLOG_WARNING << "File [" << name << "] not found";
				continue;
			}
			vector.push_back({ getLang(abrivation), name, content, {}, name });
		}
	...
	std::vector<TBindingHolder> VulkanShaderModule::createBindings(
		...
		if (!pipe) {
			PLOG_ERROR << "Shader pipe is not valid!";
			return {};
		}
		VulkanShaderPipe* shaderPipe = shaderPipes[pipe.internalHandle];
	...
	void VulkanShaderModule::addToMaterial(const graphics::Material* material,
		...
		if (!material->costumShaderData) { throw std::runtime_error("Error shader not found!"); }

		const auto vkPipe = shaderPipes[material->costumShaderData.internalHandle];

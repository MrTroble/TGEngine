...
namespace tge::gui
{
	...
	main::Error GUIModule::init()
	{
		...
		this->renderpass = (void*)VkRenderPass(vmod->device.createRenderPass(renderPassCreateInfo));
		...
		const auto result = vkCreateDescriptorPool((VkDevice)vmod->device, &pool_info, nullptr,
		...
		buffer = (VkCommandBuffer)vmod->device.allocateCommandBuffers(allocInfo).back();

		render(this);

		vmod->primary.push_back(vk::CommandBuffer((VkCommandBuffer)buffer));
	...
	void GUIModule::destroy()
	{
		...
		vmod->device.destroyDescriptorPool(vk::DescriptorPool((VkDescriptorPool)pool));
		vmod->device.destroyRenderPass(vk::RenderPass((VkRenderPass)this->renderpass));
	...
	void GUIModule::recreate()
	{
		...
		for (size_t i = 0; i < vmod->swapchainImageviews.size(); i++)
		{
			...
			const FramebufferCreateInfo framebufferCreateInfo(
				{}, vk::RenderPass((VkRenderPass)renderpass), imview, vmod->viewport.width,
